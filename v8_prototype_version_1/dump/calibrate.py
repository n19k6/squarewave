...
#             m_i = 0
#             print ("started locking")
        #print((m_min, m_max), (m, n1, n2), (m_i, m_l,d_m), (m//16, m_m//16), max(m//16, m_m//16))
        if not(old == max(m//16, m_m//16)):
            old = max(m//16, m_m//16)
            print(old)
        
#         if (m_max-m_min>20):
#             m_l = False        
        #print(measures[m_i], median)
        #m_i = (m_i+1) % 9
        #sleep_ms(43)
        t1 = ticks_ms()
        #print("exec in ms:", ticks_diff(t1, t0))
        sleep(0.1)
    #sleep(1)
    #measure(10)
    #sleep(1)
