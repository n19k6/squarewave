...
def main():
    ...
    def tick_ox1_ox2(timer):
        ...
        #so = co(so, m)
        direction = 0
        if (m4 >= (bound[ox1_freq+1]+10)):
            direction = 1
        if (m4 <= (bound[ox1_freq]-10)):
            direction = -1
        ox1_freq = ox1_freq+direction
        m4_v = m4//16
        direction2 = 0
        if ox1_freq-m4_v>2:
            direction2 = m4_v-ox1_freq
        if ox1_freq-m4_v<-2:
                direction2 = m4_v-ox1_freq
        ox1_freq = ox1_freq+direction2
        if (direction != 0) or (direction2 != 0):
            t1 = ticks_ms()
            #print(so,m1, ticks_diff(t1, t0))
            print((ox1_freq, ticks_diff(t1, t0)), m4, m4//16)
